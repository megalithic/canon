defmodule CanonWeb.SessionControllerTest do
  use CanonWeb.ConnCase

  alias Canon.Accounts
  alias Canon.Auth.Guardian

  @valid_params %{username: "some username", email: "some@email", password: "some password"}
  @invalid_attrs %{}
  ...
  describe "POST to /sessions" do
    ...
    test "logs in a user when the required params are provided", %{conn: conn, user: user} do
      ...
      expected = %{"id" => user.id, "username" => user.username, "email" => user.email}
